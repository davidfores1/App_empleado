    ...
    <div class="col-sm-10">
        <div class="form-check">
            <input class="form-check-input" type="radio" name="sexo" id="gridRadios1" value="M"
                {{ old('$empleado->sexo') === "M" ? 'checked' : (isset($empleado->boletin) && $empleado->sexo === 'M' ? 'checked' : '') }}>
        ...
        <div class="form-check">
            <input class="form-check-input" type="radio" name="sexo" id="gridRadios2" value="F"
                {{ old('$empleado->sexo') === "F" ? 'checked' : (isset($empleado->boletin) && $empleado->sexo === 'F' ? 'checked' : '') }}>
    ...
    <div class="col-sm-10">
        <select id="inputState" class="form-control" name="area_id" required>
            ...
            <option value="{{ $areas->id }}"
                {{ old('$empleado->area_id') === $areas->id ? 'selected' : (isset($empleado->area_id) && $empleado->area_id === $areas->id ? 'selected' : '')}}>
                {{ $areas->nombre_area }}</option>
            @endforeach
        </select>
        @error('area')
        <span class="invalid-feedback" role="alert">
...
<div class="form-group row">
    ...
    <div class="col-sm-10">
        <textarea required class="form-control" id="exampleFormControlTextarea1" name="descripcion"
            rows="3">{{ isset($empleado->descripcion) ?$empleado->descripcion : '' }}</textarea>
        @error('descripcion')
        <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
        </span>
...
<div class="form-group row">
    <div class="col-sm-10 offset-sm-2">
        <div class="form-check">
            <input class="form-check-input" name="boletin" type="checkbox" id="gridCheck1" value="SI"
                {{ old('$empleado->boletin') === "SI" ? 'checked' : (isset($empleado->boletin) && $empleado->boletin === 'SI' ? 'checked' : '') }}>
            <label class="form-check-label" for="gridCheck1">
